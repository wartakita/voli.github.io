...
function initializePlayer(videoId, m3u8, mp4, title) {
    ...
    Object.keys(categories).forEach(category => {
        ...
        categories[category].forEach(match => {
            ...
            listItem.innerHTML = `    
                        ...
                        <button class="btn btn-primary btn-sm watch-match" data-video-id="${match.videoId}" data-m3u8="${match.m3u8}" data-mp4="${match.mp4}" data-title="${match.teams}">Watch Match</button>    
                    `;
            listItem.addEventListener('click', () => {
                initializePlayer(match.videoId, match.m3u8, match.mp4, match.teams);
            });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('click', function(event) {
        if (event.target.classList.contains('watch-match')) {
            ...
            initializePlayer(videoId, m3u8, mp4, title);
        }
    });
});
